...
export function buildCrossword(puzzle: CrosswordPuzzle) {
  ...
  if (
    ...
    throw new Error(
      "A crossword must be a rectangular grid of letters and blocks, at most 15×15.",
    );
  }
  ...
  cells.forEach((letter, index) => {
    ...
    const startsAcross =
      ...
      (puzzle.layout !== "freeform" ||
        (column + 1 < width && cells[index + 1] !== "#"));
    const startsDown =
      (row === 0 || cells[index - width] === "#") &&
      (puzzle.layout !== "freeform" ||
        (row + 1 < height && cells[index + width] !== "#"));
    ...
    for (const direction of ["across", "down"] as const) {
      ...
      if (run.length < 2 || !clue)
        throw new Error(`Missing or invalid ${number} ${direction} clue.`);
      entries.push({
        number,
        direction,
        clue,
        cells: run,
        answer: run.map((cell) => cells[cell]).join(""),
      });
    }
  });
  ...
  if (
    !entries.length ||
    cells.some((letter, index) => letter !== "#" && !covered.has(index))
  ) {
...
export function nextSquare(
  ...
  while (next >= 0 && next < cells.length) {
    if (
      Math.abs(delta) === 1 &&
      Math.floor(next / width) !== Math.floor(index / width)
    )
      return index;
...
export function isCrosswordSolved(solution: string[], letters: string[]) {
  return solution.every(
    (letter, index) => letter === "#" || letters[index] === letter,
  );
}
...
export function nextClue(
  ...
  const index = ordered.findIndex(
    (entry) =>
      entry.number === current.number && entry.direction === current.direction,
  );
